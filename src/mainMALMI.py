...
class MALMI:
    ...
    def __init__(self, cfgs={}):
        ...
        # %% seismic parameters========================================================
        seismic = {}
        seismic['dir'] = task_dir + cfgs['MIL']['control']['dir_output']+'/data_ML/primary_events/mseeds'

            # task_dir + 'primary_events'  # path to the SDS archive directory or parent directory where seismic data are stored all in one folder
        # seismic['dir'] = cfgs['MIL']['seismic']['dir']
        seismic['instrument_code'] = cfgs['MIL']['seismic'][
            'instrument_code']  # used instrument code of the input seismic data
        seismic['stainvf'] = cfgs['MIL']['seismic']['stainvf']  # path to the station inventory file
        seismic['freqband'] = cfgs['MIL']['seismic']['freqband']
        seismic['datastru'] = 'EVS'  # the input seismic data file structure, can be 'AIO' or 'SDS' or 'EVS'
        seismic['date'] = None
        ...
        tt = {}
        tt['vmodel'] = cfgs['MIL']['tt']['vmodel']  # filename including path of the velocity model
        tt['dir'] = task_dir + cfgs['MIL']['tt']['dir']  # path to travetime data directory
        tt['build'] = cfgs['MIL']['tt'][
            'build']  # if you want to build the traveltime table or not, traveltime trable will be built when initialize MALMI class and only need to be built once
        # print(tt['vmodel'])

        # # %% migration parameters======================================================
        MIG = {}
        MIG['ppower'] = cfgs['MIL']['MIG']['ppower']
        MIG['output_migv'] = cfgs['MIL']['MIG']['output_migv']
        if MIG['ppower'] == 'None':
            MIG['ppower'] = None

        # %% detection parameters======================================================
        detect = {}
        detect['twind_srch'] = cfgs['MIL']['detect'][
            'twind_srch']  # time window length in second where events will be searched in this range
        detect['twlex'] = cfgs['MIL']['detect'][
            'twlex']  # time in second for extend the time window, roughly equal to the width of P- or S-probability envelope
        detect['P_thrd'] = cfgs['MIL']['detect'][
            'P_thrd']  # probability threshold for detecting P-phases/events from the ML-predicted phase probabilities
        detect['S_thrd'] = cfgs['MIL']['detect'][
            'S_thrd']  # probability threshold for detecting S-phases/events from the ML-predicted phase probabilities
        detect['nsta_thrd'] = cfgs['MIL']['detect'][
            'nsta_thrd']  # minimal number of stations triggered during the specified event time period
        detect['npha_thrd'] = cfgs['MIL']['detect'][
            'npha_thrd']  # minimal number of phases triggered during the specified event time period
        ...
        grid = {}
        grid['LatOrig'] = cfgs['MIL']['grid'][
            'LatOrig']  # latitude in decimal degrees of the origin point of the rectangular migration region (float, min:-90.0, max:90.0)
        grid['LongOrig'] = cfgs['MIL']['grid'][
            'LongOrig']  # longitude in decimal degrees of the origin point of the rectangular migration region (float, min:-180.0, max:180.0)
        grid['zOrig'] = cfgs['MIL']['grid'][
            'zOrig']  # Z location of the grid origin in km relative to the sea-level. Nagative value means above the sea-level; Positive values for below the sea-level;
        grid['xNum'] = cfgs['MIL']['grid']['xNum']  # number of grid nodes in the X direction (East)
        grid['yNum'] = cfgs['MIL']['grid']['yNum']  # number of grid nodes in the Y direction (North)
        grid['zNum'] = cfgs['MIL']['grid']['zNum']  # number of grid nodes in the Z direction (Vertical-down)
        grid['dgrid'] = cfgs['MIL']['grid']['dgrid']  # grid spacing in kilometers

        # %% control parameters========================================================
        control = {}
        control['dir_output'] = task_dir + cfgs['MIL']['control']['dir_output']  # path for outputs
        control['n_processor'] = cfgs['MIL']['control'][
            'n_processor']  # number of CPU processors for parallel processing

        # %% migration parameters======================================================
        MIG2 = {}
        MIG2['ppower'] = cfgs['MIL']['MIG']['ppower']
        MIG2['output_migv'] = cfgs['MIL']['MIG']['output_migv']
        if MIG2['ppower'] == 'None':
            MIG2['ppower'] = None
        ...
        self.instrument_code = copy.deepcopy(
            seismic['instrument_code'])  # the instrument codes of the input seismic data
        ...
        self.n_processor = copy.deepcopy(control['n_processor'])

        # number of threads for parallel processing
        if (self.seisdatastru == 'AIO') or (self.seisdatastru == 'EVS'):
            fd_seismic = 'primary_events'
        ...
        #     fd_seismic = 'events_input'
            ...
            raise ValueError(
                'Unrecognized input for: seisdatastru! Can\'t determine the structure of the input seismic data files!')
        ...
        self.dir_ML = os.path.join(self.dir_CML,
                                   fd_seismic)  # directory for ML outputs of the currrent processing dataset
        self.dir_mseed = os.path.join(self.dir_ML,
                                      'mseeds')  # directory for outputting seismic data for EQT, NOTE do not add '/' at the last part
        ...
        self.dir_migration = os.path.join(self.dir_MIG,
                                          fd_seismic)  # directory for migration outputs of the current processing dataset
        self.fld_prob = 'prob_evstream'  # foldername of the probability outputs of different events
        self.dir_lokiprob = os.path.join(self.dir_migration,
                                         self.fld_prob)  # directory for probability outputs of different events in SEED format
        self.dir_lokiseis = os.path.join(self.dir_migration,
                                         'seis_evstream')  # directory for raw seismic outputs of different events in SEED format
        ...
        if (tt is None) and (grid is None):
            ...
            if self.grid is not None:
                region1, mgregion = get_regioncoord(self.grid, self.stainv, 0.05,
                                                    consider_mgregion=True)  # obtain migration region lon/lat range
                self.grid['mgregion'] = copy.deepcopy(
                    mgregion)  # migration region lat/lon range, [lon_min, lon_max, lat_min, lat_max] in degree
                self.grid['pltregion'] = copy.deepcopy(
                    region1)  # plot region lat/lon range in format of [lon_min, lon_max, lat_min, lat_max] in degree
    ...
    def generate_prob(self, ML):
        ...
        if ML['engine'].lower() == 'eqtransformer':
            ...
            if self.seismic['datastru'] == 'EVS':
                ...
                event_folders = sorted([fdname for fdname in os.listdir(self.dir_seismic) if
                                        os.path.isdir(os.path.join(self.dir_seismic, fdname))])
                hdf5_evdirs = []
                prob_evdirs = []
                ...
                for ievfd in event_folders:
                    ...
                    preproc_dir_ev = os.path.join(self.dir_ML, "preproc_overlap{}".format(ML['overlap']),
                                                  ievfd)  # path of the directory where will be located the summary files generated by preprocessor step
                ...
                preproc_dir = self.dir_ML + "/preproc_overlap{}".format(ML[
                                                                            'overlap'])  # path of the directory where will be located the summary files generated by preprocessor step
            ...
            sbppara['seqtpicks'] = ML['seqtpicks']
    ...
    def event_detect_ouput(self):
        ...
        print(
            'MALMI starts to detect events based on the ML predicted phase probabilites and output the corresponding phase probabilites of the detected events:')
        ...
        if self.detect['twind_srch'] is None:
            self.detect['twind_srch'], _, _ = maxP2Stt(self.dir_tt, self.tt_hdr_filename, self.tt_ftage,
                                                       self.tt_precision)
        ...
        if self.seismic['datastru'] == 'EVS':
            ...
            for ievfd in event_folders:
                ...
                event_info = phasedetectfprob(dir_probinput=dir_prob_ev, P_thrd=self.detect['P_thrd'],
                                              S_thrd=self.detect['S_thrd'])
                ...
                arrayeventdetect(event_info=event_info,
                                 ...
                                 dir_output=self.dir_lokiprob, dir_output_seis=self.dir_lokiseis,
                                 dir_seisdataset=dir_seisdataset,
            ...
            if self.detect['nsta_min_percentage'] is not None:
                num_avasta = len([fdname for fdname in os.listdir(self.dir_prob) if os.path.isdir(
                    os.path.join(self.dir_prob, fdname))])  # get the total number of available stations
                ...
                if nsta_thrd_new > self.detect['nsta_thrd']:
                    self.detect[
                        'nsta_thrd'] = nsta_thrd_new  # reset using minimal decimal percentage of total available stations
                if npha_thrd_new > self.detect['npha_thrd']:
                    self.detect[
                        'npha_thrd'] = npha_thrd_new  # reset using minimal decimal percentage of total available phases
    ...
    def migration(self):
        ...
        inputs['normthrd'] = self.MIG[
            'probthrd']  # if maximum value of the input phase probabilites is larger than this threshold, the input trace will be normalized (to 1)
        inputs['ppower'] = self.MIG[
            'ppower']  # compute array element wise power over the input probabilities before stacking
        inputs['output_migv'] = self.MIG[
            'output_migv']  # specify whether to output the migrated data volume for each event
        ...
        l1 = Loki(data_path=self.dir_lokiprob, output_path=self.dir_lokiout, db_path=self.dir_tt,
                  hdr_filename=self.tt_hdr_filename, mode='locator')
    ...
    def rsprocess_view(self, getMLpick={}, PLT={}):
        ...
        evdir = sorted([fdname for fdname in os.listdir(self.dir_lokiout) if
                        os.path.isdir(os.path.join(self.dir_lokiout, fdname))])
        ...
        for iefd in evdir:
            ...
            if getMLpick is not None:
                get_MLpicks_ftheart(dir_prob=dir_prob_ev, dir_io=dir_output_ev, maxtd_p=getMLpick['maxtd_p'],
                                    maxtd_s=getMLpick['maxtd_s'],
                                    P_thrd=self.detect['P_thrd'], S_thrd=self.detect['S_thrd'],
                                    thephase_ftage='.phs', ofname=None, dir_seis=dir_seis_ev,
                                    snr_para=getMLpick['snr_para'])
    ...
    def clear_interm(self, CL={}):
        ...
        if CL['mseed']:
            try:
                shutil.rmtree(
                    self.dir_mseed)  # remove the mseed directory which are the formated continuous seismic data set for ML inputs
        ...
        if CL['hdf5_seis']:
            try:
                shutil.rmtree(
                    self.dir_hdf5)  # remove the hdf5 directory which are formatted overlapping data segments of seismic data set for ML_EQT predictions
            except Exception as emsg:
                print(emsg)
    ...
    def get_catalog(self, CAT={}):
        ...
        if (CAT['evselect'] is not None) and ('latitude' not in CAT['evselect']):
            if (CAT['evselect']['thrd_llbd'] is not None):
                CAT['evselect']['latitude'] = [self.grid['mgregion'][2] + CAT['evselect']['thrd_llbd'],
                                               self.grid['mgregion'][3] - CAT['evselect']['thrd_llbd']]
            else:
                CAT['evselect']['latitude'] = None

        if (CAT['evselect'] is not None) and ('longitude' not in CAT['evselect']):
            if (CAT['evselect']['thrd_llbd'] is not None):
                CAT['evselect']['longitude'] = [self.grid['mgregion'][0] + CAT['evselect']['thrd_llbd'],
                                                self.grid['mgregion'][1] - CAT['evselect']['thrd_llbd']]
            else:
        ...
        if (CAT['evselect'] is not None) and ('thrd_cmax' in CAT['evselect']) and (
                'thrd_stanum' not in CAT['evselect']):
            CAT['evselect']['thrd_stanum'] = None

        if (CAT['evselect'] is not None) and ('thrd_cmax' in CAT['evselect']) and (
                'thrd_phsnum' not in CAT['evselect']):
