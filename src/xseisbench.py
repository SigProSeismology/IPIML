...
#     return dattime, probability
#
# def add_seqt_picks(annotations, direct,ev_id):
#     e_dict = np.load(direct, allow_pickle=True)
#     PICKS = e_dict.item()[str(ev_id)]
#     # Access the dictionary with the specified ID
#
#     # Iterate over the 'Picks' list
#     netsta=annotations.traces[0].stats.network+'.'+annotations.traces[0].stats.station
#     if annotations.traces[0].id is not None:
#         WT=annotations.traces[0].id
#         wave_type=WT[-1]
#         get_probability(PICKS, netsta, wave_type)
#
#
#
#     utc_datetime = UTCDateTime(dattime)
#     new_amplitude = probability
#     component_type = 'P'
#
#     # Find the trace with the specified component type
#     matching_trace = None
#     for trace in stream:
#         if 'EQTransformer_{}'.format(component_type) in trace.stats.channel:
#             matching_trace = trace
#             break
#
#     # Check if a matching trace is found
#     if matching_trace is not None:
#         if matching_trace.stats.starttime <= utc_datetime <= matching_trace.stats.endtime:
#             # Calculate the index corresponding to the UTCDateTime in the trace
#             index = int((utc_datetime - matching_trace.stats.starttime) * matching_trace.stats.sampling_rate)
#
#             # Replace the amplitude at the specified index
#             matching_trace.data[index] = new_amplitude
#
#     print(stream)
#
#     # Specify the UTCDateTime, the corresponding amplitude, and the component type
#
#     aaa=1
#
#     # value = e_dict.item()['1']['Picks'][2][3]
#     # picck=e_dict.item()[str(ev_id)]['Picks']
#     # print(picck)
#     # e_dict[ev_id]['Picks']
#     # e_dict['ev_id']
#     # ed=e_dict[ev_id]
