...
def seischar_plot(dir_seis, dir_char, dir_output, figsize=(12, 12), comp=['Z','N','E'], dyy=1.8, fband=None, normv=None, ppower=None, tag=None, staname=None, arrvtt=None, timerg=None, dpi=300, figfmt='png', process=None, plotthrd=None, linewd=0.6, problabel=False, yticks='auto', ampscale=1.0):
    ...
    for icomp in comp:
        ...
        for ii in range(Nsta):
            ...
            if charfs is not None:
                ...
                if tr.count() > 0:
                    ...
                    if problabel:
                        if timerg is not None:
                            ax.text(timerg[-1], ydev[ii]+0.35*dyy, 'P_prob_max: {:.3f}'.format(dampmax), color='r', fontname='Arial', fontsize=13, fontweight='bold', va='bottom', ha='right')
                        else:
                            ax.text(tt[-1], ydev[ii]+0.35*dyy, 'P_prob_max: {:.3f}'.format(dampmax), color='r', fontname='Arial', fontsize=13, fontweight='bold', va='bottom', ha='right')
                    del vdata, tt
                del tr
                ...
                if tr.count() > 0:
                    ...
                    if problabel:
                        if timerg is not None:
                            ax.text(timerg[-1], ydev[ii]+0.19*dyy, 'S_prob_max: {:.3f}'.format(dampmax), color='b', fontname='Arial', fontsize=13, fontweight='bold', va='bottom', ha='right')
                        else:
                            ax.text(tt[-1], ydev[ii]+0.19*dyy, 'S_prob_max: {:.3f}'.format(dampmax), color='b', fontname='Arial', fontsize=13, fontweight='bold', va='bottom', ha='right')
                    del vdata, tt
                del tr
        ...
        plt.xticks(fontname='Arial', fontsize=16)  # , fontweight ="bold"
        if yticks == 'station':
            ax.set_yticks(ydev)
            ax.set_yticklabels(staname, fontname='Arial', fontsize=16)  # , fontweight ="bold"
        elif yticks == 'index':
            ytck = ydev[0:-1:10]
            ytck_lb = [int(iyk/dyy+1) for iyk in ytck]
            ax.set_yticks(ytck)
            ax.set_yticklabels(ytck_lb, fontname='Arial', fontsize=16)  # , fontweight ="bold"
            ax.set_ylabel('Station number', fontname='Arial', fontsize=16)  # , fontweight ="bold"
        ax.tick_params(axis='x', labelsize=16)
        ax.set_title('Data [{}]'.format(this_date), fontname='Arial', fontsize=16, fontweight ="bold")
        if tag:
...
def migmatrix_plot(file_corrmatrix, dir_tt, hdr_filename='header.hdr', colormap='RdBu_r', dir_output=None, figfmt='png', normrg=None, plotstyle='contourf'):
    ...
    ax.set_xlabel('East (Km)', fontname='Arial', fontsize=16)  # , fontweight='bold'
    ax.set_ylabel('North (Km)', fontname='Arial', fontsize=16)  # , fontweight='bold'
    ...
    ax.set_title('Migration profile East-North', fontname='Arial', fontsize=16)  # , fontweight='bold'
    ...
    fig.suptitle('Migration profile East-North', fontname='Arial', fontsize=16)  # , fontweight='bold'
    ax = fig.gca()
    ...
    ax.set_xlabel('East (Km)', fontname='Arial', fontsize=16)  # , fontweight='bold'
    ax.set_ylabel('Depth (Km)', fontname='Arial', fontsize=16)  # , fontweight='bold'
    # make the grid lines transparent
    ax.xaxis._axinfo["grid"]['color'] =  (1,1,1,0)
    ax.yaxis._axinfo["grid"]['color'] =  (1,1,1,0)
    ax.zaxis._axinfo["grid"]['color'] =  (1,1,1,0)
    ...
    ax.yaxis.set_pane_color((1.0, 1.0, 1.0, 0.0))
    ax.zaxis.set_pane_color((1.0, 1.0, 1.0, 0.0))
    ax.set_title('Migration profile East-Depth', fontname='Arial', fontsize=16)  # , fontweight='bold'
    ...
    fig = plt.figure(dpi=600)
    fig.suptitle('Migration profile East-Depth', fontname='Arial', fontsize=16)  # , fontweight='bold'
    ax = fig.gca()
    if plotstyle == 'contourf':
    ...
    ax.set_xlabel('North (Km)', fontname='Arial', fontsize=16)  # , fontweight='bold'
    ax.set_ylabel('Depth (Km)', fontname='Arial', fontsize=16)  # , fontweight='bold'
    # make the grid lines transparent
    ax.xaxis._axinfo["grid"]['color'] =  (1,1,1,0)
    ax.yaxis._axinfo["grid"]['color'] =  (1,1,1,0)
    ...
    ax.yaxis.set_pane_color((1.0, 1.0, 1.0, 0.0))
    ax.zaxis.set_pane_color((1.0, 1.0, 1.0, 0.0))
    ax.set_title('Migration profile North-Depth', fontname='Arial', fontsize=16)  # , fontweight='bold'
    ...
    fig = plt.figure(dpi=600)
    fig.suptitle('Migration profile North-Depth', fontname='Arial', fontsize=16)  # , fontweight='bold'
    ax = fig.gca()
    if plotstyle == 'contourf':
    ...
    if sta_inv:
        for net in sta_inv:
            for sta in net:
                ...
                if plot_stationname:
                    fig.text(text=sta.code, x=sta.longitude, y=sta.latitude, font='6p,Arial-Bold,black', justify='CT', offset='0/-0.15c')
...
def plot_evmap_depth(region, eq_longi, eq_latit, eq_depth, depthrg=None, cmap="polar", sta_inv=None, mkregion=None, fname="./basemap.png", plot_stationname=False, eq_size=0.17):
    ...
    for net in sta_inv:
        for sta in net:
            ...
            if plot_stationname:
                fig.text(text=sta.code, x=sta.longitude, y=sta.latitude, font='6p,Arial-Bold,black', justify='CT', D='0/-0.15c')
    ...
    fig.text(text='{} events'.format(len(eq_longi)), position='BR', font='14p,Arial-Bold,black', justify='BR', offset='-0.4/0.4')
