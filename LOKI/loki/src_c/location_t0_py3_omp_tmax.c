...
/* Prototypes */
int stacking(long int nxyz, long int nsta, long int nsamples, int itp[nxyz][nsta], int its[nxyz][nsta], double stalta_p[nsta][nsamples], double stalta_s[nsta][nsamples], double corrmatrix[nxyz], double tmax[nsamples], long int *iloc, long int *itime, int nproc);
...
static PyObject *py_stacking(PyObject *self, PyObject *args){
   PyArrayObject *itp, *its, *stalta_p, *stalta_s, *corrmatrix, *tmax;
   ...
   dims2[0] = nsamples;
   tmax=(PyArrayObject*) PyArray_SimpleNew(1, dims2, NPY_DOUBLE);

   /*call stacking */
   if (0 != stacking(nxyz, nsta, nsamples, PyArray_DATA(itp), PyArray_DATA(its), PyArray_DATA(stalta_p), PyArray_DATA(stalta_s), PyArray_DATA(corrmatrix), PyArray_DATA(tmax), &iloc, &itime, nproc)) {
	 ...
	 Py_DECREF(corrmatrix);
         ...
         PyObject *cohertim=Py_BuildValue("O",tmax);
	 Py_DECREF(tmax);

	 PyObject *locres=Py_BuildValue("OOO",iloctime, cohermat, cohertim);
	 Py_DECREF(iloctime);
	 Py_DECREF(cohermat);
         Py_DECREF(cohertim);
...
static struct PyModuleDef modlocation_t0_tmax = {
...
PyMODINIT_FUNC PyInit_location_t0_tmax(void){
    PyObject *m;
    m = PyModule_Create(&modlocation_t0_tmax);
    if (m==NULL)
       return NULL;
    import_array();
    return m;
...
int stacking(long int nxyz, long int nsta, long int nsamples, int itp[nxyz][nsta], int its[nxyz][nsta], double stalta_p[nsta][nsamples], double stalta_s[nsta][nsamples], double corrmatrix[nxyz], double tmax[nsamples], long int *iloc, long int *itime, int nproc){
    ...
    long int iter, i, j, k, kmax, iit;
    int ip, is;
    double stk0p, stk0s, stkmax, corrmax;
    iter=0;
    corrmax=-1.0;
    for (iit=0;iit<nsamples;iit++){
        tmax[iit]=-1.0;
    }
    ...
    for(i=0;i<nxyz;i++){
       ...
       for(k=0;k<nsamples;k++){
           ...
           #pragma omp critical
           if (sqrt(stk0p*stk0s)/((float) nsta)>tmax[k]){
               tmax[k]=sqrt(stk0p*stk0s)/((float) nsta);
           }
       }
