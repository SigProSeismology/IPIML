...
build_taup_model("ghana.nd",output_folder="./", verbose=False)
model = TauPyModel(model="ghana")
# model = TauPyModel(model="ak135")

with open("ttdb3.txt", "w") as f:
    #f.write("dist dep tp ts tp_slowness ts_slowness tp_hslowness ts_hslowness p_elvecorr s_elvecorr\n")
    for dep in range(0,800,5): # search 0 - 800 km in depth
        for dist in range(1,600,3): # search 0.01 - 1 degree in horizontal 
            dist = dist*0.01
            print(dep,dist)
